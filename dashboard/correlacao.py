...
OPCOES_CORRELACAO = [
    ...
    {"label": "O Idioma Original é Inglês?", "value": "is_original_en"},
    {"label": "Possui Dublagem Em Inglês?", "value": "has_spoken_en"},
    {"label": "Qtd. Países Produtores", "value": "num_countries"},
    {"label": "Qtd. Empresas Produtoras", "value": "num_companies"}
]
    ...
    return html.Div([
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Dropdown(
                    ...
                    # Seleção padrão para não iniciar vazio
                    value=["budget", "revenue", "popularity", "vote_average", "is_original_en"], 
                    multi=True,
        ...
        dbc.Row([
            dbc.Col([
                ...
                ], outline=True, color="danger", className="shadow-sm mb-5")
            ], width=12)
        ])
...
def register_correlation_callbacks(app, df):
    ...
    def atualizar_grafico_linha_impacto(coluna_x, coluna_y, filtro_checkbox):
        ...
        # 1. Copia o dataframe para tratamento
        df_plot = df.copy()

        # 2. Processa as variáveis categóricas
        for col in [coluna_x, coluna_y]:
            if col == "is_original_en":
                df_plot["is_original_en"] = (df_plot["original_language"] == "en").astype(int)
            elif col == "has_spoken_en":
                df_plot["has_spoken_en"] = df_plot["spoken_languages"].fillna("").str.contains("en", case=False).astype(int)
            elif col == "num_countries":
                df_plot["num_countries"] = df_plot["production_countries"].fillna("").apply(
                    lambda x: len(str(x).split(",")) if str(x).strip() else 0
                )
            elif col == "num_companies":
                df_plot["num_companies"] = df_plot["production_companies"].fillna("").apply(
                    lambda x: len(str(x).split(",")) if str(x).strip() else 0
                )

        # 3. Lógica do Checkbox: Filtrar ou Manter Zeros/Nulos
        ignorar_zeros = filtro_checkbox and "ativado" in filtro_checkbox

        if ignorar_zeros:
            # Remove linhas nulas permanentemente
            df_plot = df_plot.dropna(subset=[coluna_x, coluna_y])
            # Se for variável financeira, o zero indica ausência de dado, então removemos
            if coluna_x in ["budget", "revenue", "runtime", "popularity", "vote_average", "vote_count", "num_countries", "num_companies"]:
                df_plot = df_plot[df_plot[coluna_x] > 0]
            if coluna_y in ["budget", "revenue", "runtime", "popularity", "vote_average", "vote_count", "num_countries", "num_companies"]:
                df_plot = df_plot[df_plot[coluna_y] > 0]
        else:
            # Se for para usar os dados "sujos", convertemos NaN para 0 para que a matemática 
            # de agrupamento funcione e o Plotly possa renderizar o ponto no gráfico.
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="lines+markers",
            line=dict(color="#d8042a", width=4), 
            marker=dict(size=10, color="#d8042a", symbol="circle"),
        ...
        fig.update_layout(
            ...
            yaxis_title=f"<b>{mapa_nomes[coluna_y]} (Média)</b>",
            plot_bgcolor="#f8f9fa",
            paper_bgcolor="white",
    ...
    def atualizar_grafico_correlacao(colunas_selecionadas, filtros_checkbox):
        ...
        # 2. Engenharia de Recursos Dinâmica (Tratamento das Categóricas)
        
        # Original Language: 1 se for 'en', 0 caso contrário
        if "is_original_en" in colunas_selecionadas:
            df_plot["is_original_en"] = (df_plot["original_language"] == "en").astype(int)
            
        # Spoken Languages: 1 se contém 'en', 0 caso contrário
        if "has_spoken_en" in colunas_selecionadas:
            df_plot["has_spoken_en"] = df_plot["spoken_languages"].fillna("").str.contains("en", case=False).astype(int)
            
        # Production Countries: Conta quantos países estão envolvidos
        if "num_countries" in colunas_selecionadas:
            df_plot["num_countries"] = df_plot["production_countries"].fillna("").apply(
                lambda x: len(str(x).split(",")) if str(x).strip() else 0
            )
            
        # Production Companies: Conta quantas empresas estão envolvidas
        if "num_companies" in colunas_selecionadas:
            df_plot["num_companies"] = df_plot["production_companies"].fillna("").apply(
                lambda x: len(str(x).split(",")) if str(x).strip() else 0
            )

        # 3. Lógica do Checkbox Aplicada às Colunas Numéricas (Preservando as Flags de Idioma)
        ...
        if ignorar_zeros:
            ...
            # Identifica colunas de idioma para não deletar os valores 0 delas
            colunas_booleanas = ["is_original_en", "has_spoken_en"]
            for c in colunas_selecionadas:
                df_plot[c] = pd.to_numeric(df_plot[c], errors='coerce')
                if c not in colunas_booleanas:
        ...
        # Prevenção contra dataframe vazio após os filtros aplicados
        ...
        # Para exibir os nomes amigáveis no gráfico ao invés dos nomes do banco de dados
        mapa_nomes = {item["value"]: item["label"] for item in OPCOES_CORRELACAO}
        df_corr.rename(index=mapa_nomes, columns=mapa_nomes, inplace=True)

        escala_customizada = [
            [0.0, "#fce4ec"],   # -1 (Rosa neutro muito claro/desbotado)
            [0.5, "#f4f6f6"],   #  0 (Nula: Cinza claro/Quase branco)
            [0.75, "#ea6d82"],  #  0.5 (Moderada: Vermelho fechado)
            [1.0, "#d8042a"]    #  1.0 (100% de Correlação: Vermelho escuro profundo / Vinho)
        ]

        # 5. Cria o Mapa de Calor (Heatmap) com Plotly
        fig = px.imshow(
            df_corr,
            text_auto=".2f", # Mostra 2 casas decimais no bloco
            aspect="auto",
            color_continuous_scale=escala_customizada, 
            zmin=-1, # Limite mínimo de correlação
            zmax=1,  # Limite máximo de correlação
        ...
        fig.update_layout(
            ...
            # Estilização dos eixos para forçar o Negrito (Bold) nos labels
            xaxis=dict(
            ...
            # Garante que os números dentro dos quadradinhos também fiquem em negrito
            font=dict(weight="bold"),
            plot_bgcolor="white"
        )
